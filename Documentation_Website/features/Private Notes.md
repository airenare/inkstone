...
Share the note as `/draft-post?token=mysecret`. Once a visitor visits with the correct token, their session tracks that URL as unlocked, so they can navigate away and return without needing the token again.
