...
- **Canvas boards** — name a `.canvas` file `Your Title__website.canvas` to publish it (Obsidian strips JSON flags on save; the filename marker is durable); legacy `"website": true` in the JSON still works; interactive pan/zoom/fit-to-view; wide-view button (⛶) expands the canvas to fill the browser window (Esc to exit); text nodes render full markdown (callouts, tables, wiki-links, Dataview); edges support `fromEnd`/`toEnd` directionality; file cards can show a scrollable preview of linked published notes; link nodes show domain + ↗ icon; groups get a color tint
