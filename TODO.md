...
- [ ] **[UX][a11y][Group M] Add `datetime` attribute to `<time>` elements in feed and feed embeds** — `feed.html` and `base.css`-rendered feed entries use `<time class="feed-entry-date">` with only localized display text and no `datetime="YYYY-MM-DD"` attribute. This makes the date unparseable by assistive technology and structured-data consumers. Fix: pass `post.date.strftime("%Y-%m-%d")` into a `datetime` attribute on every `<time>` element across `feed.html`, `post.html` (post-meta), and any note-embed templates.
